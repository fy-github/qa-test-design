...
- For formal test-report docx deliverables, headings must use real Word paragraph styles (`Title` / `Heading 1` / `Heading 2` / `Heading 3`), never manual bold text masquerading as headings; follow the typography and verification rules in [references/output-formats.md](references/output-formats.md) (`### 正式测试报告（docx）排版规则`).
